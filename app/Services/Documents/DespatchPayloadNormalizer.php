...
final class DespatchPayloadNormalizer
{
    ...
    public function request(array $payload): array
    {
        ...
        try {
            ...
            if (! $reason || ! $mode) {
                $this->invalid('Invalid despatch reason or transport mode.');
            }
            ...
            if (! is_array($goods) || $goods === []) {
                $this->invalid('At least one transported good is required.');
            }
            $goods = array_map(function ($item) {
                if (! is_array($item) || trim((string) ($item['descripcion'] ?? '')) === '') {
                    $this->invalid('Every good requires a description.');
                }
                $unit = strtoupper((string) ($item['unidad'] ?? ''));
                if (! in_array($unit, self::UNITS, true)) {
                    $this->invalid('Unsupported unit of measure.');
                }
            ...
            if ($mode === DespatchTransportMode::Public && ! $carrier) {
                $this->invalid('Public transport requires a carrier.');
            }
            if ($mode === DespatchTransportMode::Private && (! $driver || ! $vehicle)) {
                $this->invalid('Private transport requires a driver and vehicle.');
            }
            $sender = isset($payload['remitente']) ? $this->party($payload['remitente'], 'remitente') : null;
            if ($type === DocumentType::CarrierDespatch && ! $sender) {
                $this->invalid('GRE transportista requires the sender party.');
            }
            $related = array_map(function ($doc) {
                if (! is_array($doc) || empty($doc['tipo']) || empty($doc['numero'])) {
                    $this->invalid('Related documents require type and number.');
                }
    ...
    private function party(mixed $party, string $field, bool $mtc = false): array
    {
        ...
        if ($mtc && isset($party['registro_mtc']) && trim((string) $party['registro_mtc']) === '') {
            $this->invalid('registro_mtc cannot be blank.');
        }
    ...
    {
        if (! is_array($address) || ! preg_match('/^[0-9]{6}$/D', (string) ($address['ubigeo'] ?? '')) || trim((string) ($address['direccion'] ?? '')) === '') {
            $this->invalid("{$field} requires ubigeo and address.");
        }
    ...
    {
        if (! is_array($driver) || empty($driver['tipo_documento']) || empty($driver['numero_documento']) || empty($driver['nombres']) || empty($driver['apellidos']) || empty($driver['licencia'])) {
            $this->invalid('Driver identity, name and licence are required.');
        }
    ...
    {
        if (! is_array($vehicle) || ! preg_match('/^[A-Z0-9-]{5,10}$/D', strtoupper((string) ($vehicle['placa'] ?? '')))) {
            $this->invalid('A valid vehicle plate is required.');
        }
