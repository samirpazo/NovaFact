...
class InvoicePdfService
{
    public function generateTicketFromStored(object $document, iterable $lines, ?object $company = null): string
    {
        ...
        $ruc = $company?->McrRuc ?? '';
        $qr = $this->ticketQr(implode('|', [$ruc, $document->McrDocumentType, $document->McrSeriesCode, $document->McrCorrelative, $document->McrTaxAmount, $document->McrTotalAmount, $document->McrIssueDate, $document->McrCustomerDocumentType, $document->McrCustomerDocumentNumber]));
...
<div class="center company">'.$e($company?->getRazonSocial()).'</div><div class="center">RUC '.$e($company?->getRuc()).'</div><div class="center">'.$e($company?->getNombreComercial()).'</div><div class="center">'.$e($address?->getDireccion()).'</div><div class="center">Código de local: '.$e($address?->getCodLocal()).'</div>
...
<div class="rule"></div><div class="label">'.$e($legend).'</div><div class="label">FORMA DE PAGO: CONTADO</div><div class="label">Observaciones:</div><div class="qr"><img src="data:image/svg+xml;base64,'.base64_encode($qrSvg).'" alt="QR"></div><div class="footer">Representación impresa del comprobante electrónico<br>Consulte la validez del comprobante en SUNAT<br>¡Gracias por su preferencia!</div></body></html>';
    }
